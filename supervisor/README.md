...
- `init_permissions.sh`
  ...
  - Publishes the git-master `./brain` folder to `/srv/ugc-pipeline/brain` because dropped users cannot access files under `/root`.

- `submit_request.sh <client-id> <pokemon-name>`
  - Creates the Linux user on first request.
  - Creates the private client folder under `/srv/ugc-pipeline/clients/<client-id>`.
...
- `collect_learnings.sh [output-path]`
  - Reads all `learning.md` and `learnings.md` files under `/srv/ugc-pipeline/clients`.
  - Writes an aggregate report, defaulting to `/srv/ugc-pipeline/all_learnings.md`.
