# Aggregated Learnings

Generated at 2026-04-27T15:18:15Z.

## /srv/ugc-clients/usera/requests/20260427T151651868545534Z-image/learning.md

# Learning

- Process B mode: text_to_image
- Model: `fal-ai/nano-banana-2`
- Images returned by fal: 1
- Images downloaded locally: 1
- Process C should verify the generated image matches the prompt and contains no disallowed nudity.

## /srv/ugc-clients/userb/requests/20260427T151723731458273Z-image/learning.md

# Learning

- Process B mode: image_edit
- Model: `fal-ai/nano-banana-2/edit`
- Images returned by fal: 1
- Images downloaded locally: 1
- Process C should verify the generated image matches the prompt and contains no disallowed nudity.

